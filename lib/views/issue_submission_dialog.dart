...
class IssueSubmissionDialog extends StatelessWidget {
  const IssueSubmissionDialog({Key? key, required this.issue}) : super(key: key);
  final Issue issue;

  Future<bool> submitReport(String token) async {
    await Future.delayed(const Duration(seconds: 1));
    return IssueApi().submitReport(issue, token);
  }
